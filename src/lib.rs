...
pub async fn start() {
    ...
    run().await.expect("Failed to run webview");
}

async fn run() -> Result<(), Box<dyn std::error::Error>> {
    log::info!("run().");
    let event_loop = winit::event_loop::EventLoop::new()?;
    log::info!("created window event loop.");
    ...
    // 需要确保 canvas 设置了尺寸后才能设置 surface，否则会 panic
    let mut surface_configured = false;

    log::info!("Entering render loop...");
    let _ = winit::event_loop::EventLoop::run(event_loop, move |event, target| {
        match event {
